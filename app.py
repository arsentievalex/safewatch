...
st.markdown("<div class='netflix-header'>SafeWatch</div>", unsafe_allow_html=True)
st.header("Personalized trigger warnings for a safer viewing experience")
st.write('')
st.write('')
    ...
    if st.button("Add"):
        ...
        if is_valid_url(new_video_url):
            with st.spinner("Processing a new video..."):
                # Download the video
                file_path = download_youtube_video(new_video_url)
                ...
                if analyzer_response:
                    ...
                    save_json(analyzer_response, f"{file_name}.json")
                ...
                add_entry_json("processed_videos.json", new_entry)
            st.rerun()
...
def initialize_video_data():
    ...
    if 'videos' not in st.session_state:
        ...
        for video in processed_videos:
            # Use URL as the key for easy referencing
            url = video['url']
            
            # Parse triggers for each video
            unique_triggers, filtered_events = parse_json_triggers(video['json_file'])
